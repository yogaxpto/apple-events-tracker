...
- [.github/workflows/](.github/workflows/) — `update.yml` (daily cron + dispatch pipeline; conditional fetch means most runs only refresh time-based status, and it deploys its own bot commits in-run), `pages.yml` (publishes `docs/` to Pages on a human push to `main` — bot pushes use `GITHUB_TOKEN` and don't trigger it, so no double-deploy), and `ci.yml` (lint/type/test on push).
